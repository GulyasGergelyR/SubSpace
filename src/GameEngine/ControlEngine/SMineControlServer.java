...
public class SMineControlServer extends SControlServer<SDebris>{
	public SMineControlServer(SDebris mobile){
		super(mobile);
	}
	@Override
		...
		for(SEntity entity : SFH.Entities.getObjects()){
			if (entity.getObjectState().equals(ObjectState.Active) &&
				...
				if (interaction.IsHappened()){
						...
						SM message = SMPatterns.getObjectUpdateMessage((SMine)Owner);
